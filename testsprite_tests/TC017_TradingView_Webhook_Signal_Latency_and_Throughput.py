...
async def run_test():
    ...
    try:
        ...
        # -> Click the Connect button to proceed to the next step where webhook testing can be initiated.
        ...
        # -> Input Operator ID and Access Key, then click Sign In to authenticate.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Webhook Signal Processed Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: The webhook endpoint did not handle the high-frequency TradingView signals as expected. Signals may have been lost or delayed, or the system became unresponsive under load.")
