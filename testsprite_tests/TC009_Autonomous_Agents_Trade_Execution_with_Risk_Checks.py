...
async def run_test():
    ...
    try:
        ...
        # -> Click the Connect button to establish connection and proceed to trading interface
        ...
        # -> Input a valid Google account email or phone number and click Next to continue authentication
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Trade Executed Successfully').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: Autonomous trading agents did not execute trades correctly after passing risk management constraints enforced by RiskEngine, FrictionSimulator, and RegimeDetector.")
