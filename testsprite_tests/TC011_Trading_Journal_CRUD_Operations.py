...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to the trading journal page
        ...
        # -> Input Operator ID and Access Key, then click Sign In to access the trading journal page
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Journal entry successfully created and analytics updated').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Unable to verify creation, update, and deletion of journal entries with corresponding analytics updates as per the test plan.')
