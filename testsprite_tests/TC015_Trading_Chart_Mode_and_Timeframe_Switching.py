...
async def run_test():
    ...
    try:
        ...
        # -> Click the Connect button to access the trading chart on dashboard.
        ...
        # -> Input Operator ID and Access Key, then click Sign In to proceed to dashboard.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Trading chart mode: Quantum Flux').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Trading charts did not switch modes or update correctly as per the test plan.')
