# AutoResearch Session: INTEGRATION_TESTER
# Terminal: 6
...
```bash
# 1. Create branch
git checkout -b autoresearch/integration_tester/overnight-2026-03-22
...
Path: `.claude/overnight/terminal_6_status.json`
```json
{
  "status": "running",
  "role": "INTEGRATION_TESTER",
...
Path: `.claude/overnight/terminal_6_heartbeat.jsonl`
```json
...
```bash
cat .claude/overnight/commands/terminal_6_queue.json 2>/dev/null || echo '{"commands":[]}'
```
